...
async function extractTextFromPDF(buffer: Buffer): Promise<ProcessedDocument> {
  try {
    ...
    // Use a promise-based dynamic import to handle pdf-parse
    const pdfData = await new Promise<any>(async (resolve, reject) => {
      try {
        // Dynamically import pdf-parse
        const pdfModule = await import('pdf-parse');
        
        // Try different ways to access the parsing function
        let parseFunction: any;
        if (typeof pdfModule === 'function') {
          parseFunction = pdfModule;
        } else if ('default' in pdfModule && typeof (pdfModule as any).default === 'function') {
          parseFunction = (pdfModule as any).default;
        } else if (typeof (pdfModule as any).PDFParse === 'function') {
          // If it's the class, we need to instantiate it
          const parser = new (pdfModule as any).PDFParse({ data: buffer });
          const result = await parser.getText();
          resolve(result);
          return;
        } else {
          // Try calling the module directly if it's callable
          try {
            const result = await (pdfModule as any)(buffer);
            resolve(result);
            return;
          } catch (callError) {
            reject(new Error(`Could not find callable PDF parsing function. Available exports: ${Object.keys(pdfModule).join(', ')}`));
            return;
          }
        }
        ...
        reject(new Error(`Failed to import or use pdf-parse: ${importError.message}`));
      }
    });
