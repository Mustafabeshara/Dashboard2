 ...
 */
const getGeminiConfig = async (): Promise<{
  apiKey: string;
  apiUrl: string;
  ...
  const geminiKey =
    (await getGeminiApiKey()) || process.env.GEMINI_API_KEY || process.env.GOOGLE_AI_API_KEY;
  ...
  const forgeKey =
    (await getForgeApiKey()) || process.env.FORGE_API_KEY || process.env.OPENAI_API_KEY;
  const apiUrl = process.env.FORGE_API_URL || process.env.OPENAI_API_BASE;

  if (!forgeKey) {
    throw new Error(
      'No AI API key configured. Please set GEMINI_API_KEY, GROQ_API_KEY, or configure via Settings > AI Provider API Keys.'
    );
  }
 ...
 * Invoke Groq API
 */
export async function invokeGroq(params: InvokeParams & { model?: string }): Promise<InvokeResult> {
  ...
  const { messages, tools, toolChoice, tool_choice, model = 'llama-3.1-70b-versatile' } = params;
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`Groq invoke failed: ${response.status} ${response.statusText} – ${errorText}`);
  }
