...
// Schema for individual tender items
...
export const tenderExtractionSchema = z.object({
  reference: z.string().min(1, 'Tender reference is required').max(50, 'Reference must be less than 50 characters'),
  title: z.string().min(1, 'Tender title is required').max(200, 'Title must be less than 200 characters'),
...
export function sanitizeTenderExtraction(data: any): Partial<TenderExtraction> {
  ...
  if (Array.isArray(data.items)) {
    sanitized.items = data.items
      ...
      .map((item: any) => ({
        itemDescription: typeof item.itemDescription === 'string' 
  ...
  if (data.confidence && typeof data.confidence === 'object') {
    ...
    sanitized.confidence = {
      ...
      reference: typeof conf.reference === 'number' ? Math.min(1, Math.max(0, conf.reference)) : 0.5,
      title: typeof conf.title === 'number' ? Math.min(1, Math.max(0, conf.title)) : 0.5,
      organization: typeof conf.organization === 'number' ? Math.min(1, Math.max(0, conf.organization)) : 0.5,
      closingDate: typeof conf.closingDate === 'number' ? Math.min(1, Math.max(0, conf.closingDate)) : 0.5,
